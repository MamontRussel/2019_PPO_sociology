...
'''
Вещественные числа в питоне обозначаются словом float. Вещественные числа записываются через точку. 
Также могут быть представлены в виде математической записи: 1/10000 = 1e-05
'''
import math 
...
'''
В памяти компьютера вещественные числа представляются в виде степеней двойки. К
сожалению, не все числа точно представимы в виде степеней двойки. 
Как следствие, получаем не совсем точное представление числа. Рассмотрим на примере.
'''
...
'''
Теперь представим, что наш сайт переехал в Казахстан и мы хотим поменять ru на kz. 
Можно найти индекс подстроки 'ru', выбрать подстроку с начала исходной и до 'ru', а затем прибавить 'kz'.
'''
idx = web.find('ru') # находим
print(web[:idx] + 'kz') # прибавляем

'''
Но что если в адресе сайта тоже будет посдстрока 'ru', например 'www.rumandwine.ru', 
но мы не хотели бы ее менять? Априори мы знаем, что национальный домен сайта находится в конце, т.е. справа.
'''
